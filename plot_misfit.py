...
if mistype == "data_sourcewise":
    ...
    for plotno,ridge in enumerate(ridges[:4]):
        ...
        for src in range(1,num_source+1):
            ...
            plt.semilogy(list(range(num_misfit_files)),modemisfit[plotno,:,src-1],'o-',label="x="+str(int(srclocs[src-1]))+" Mm")
    ...
    for sp in range(subplot_layout[1]):
    ...
    for ridgeno,ridge in enumerate(ridges[:4]):
        ...
        plt.semilogy(list(range(num_misfit_files)),modemisfit[ridgeno],marker=next(markers),color='black',
    ...
    plt.semilogy(list(range(num_misfit_files)),total_misfit,color='black',marker='o',ls='solid')
    plt.tick_params(axis='both', which='major', labelsize=14)
    plt.xlim(-0.5,num_misfit_files+5)
    plt.xlabel("Iteration",fontsize=20)
    ...
    num_lines = np.loadtxt(misfit_all_files[0],comments="#",usecols=list(map(int,ridges))).shape[0]
    ...
    with open(misfit_all_files[0],'r') as mf:
        ...
        for i in range(len(band_lines)): band_lines[i]=band_lines[i].strip().split()
        num_rec_line = all_lines[1].strip().split()
    
    for line in band_lines: line.remove("#")    
    num_rec_line.remove("#")

    freqband_list=["{:2.1f} to {:2.1f}".format(float(band_lines[i][1]),float(band_lines[i][2])) for i in range(num_freq_bands)]
    ...
    for iterno,misfit_all_file in enumerate(misfit_all_files):
        ...
        for ridgeno,ridge in enumerate(ridges):
            ...
            for freqband in range(num_freq_bands):
    ...
    for ridgeno,ridge in enumerate(ridges):
        ...
        linestyles = itertools.cycle(('solid','dashed','dotted'))
        for freqband in range(num_freq_bands):
            ax[ridgeno].semilogy(list(range(num_misfit_all_files)),modemisfit[ridgeno,:,freqband],marker=next(markers),color='black',
    ...
    if sound_speed_perturbed:
        ...
        plt.plot(list(range(len(modelmisfit_list))),modelmisfit_list,linestyle='solid',marker='^',label="$c$",color='black')
