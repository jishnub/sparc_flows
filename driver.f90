...
!~     stop
    ...
    if (COMPUTE_DATA) then
        ...
        if (FLOWS) then
            ...
            if (contrib=="01") then
                print *,maxval(v0_x)*dimc*10.**(-2.),maxval(v0_z)*dimc*10.**(-2.)
        ...
        if (FLOWS) then
            ...
            if (iteration) then
                ...
                print *,"psivar max",maxval(psivar)
                if (contrib=="01") call writefits_3d('psivar_'//jobno//'.fits',psivar,nz)
                ...
                if (contrib=="01") then
                    print *, "vxmax",maxval(v0_x*dimc*10.**(-2.)) &
                    ,"vzmax", maxval(v0_z*dimc*10.**(-2.))
            ...
            do i=1,nx
                if ((distances(i) > mindist) .and. (distances(i) < maxdist)) then
                    ...
                    if (.not. linesearch) then
                        ...
                        lef = loc - halftime
                        rig = loc + halftime
!~                         print *,i,lef,rig,timest,timefin,pord
...
138                 format (I3,X,F14.2,X,I4,X,I4,X,I4,X,I4,X,I4)
