import os,sys,shutil,glob,time,re,subprocess,datetime
...
print "Running linesearch no",linesearch_no,"for src no",src_no,"on proc",procno+1,"node no",nodeno+1,"at time",datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')
...
def compute_forward(linesearch_no,src):
    ...
    mpipath=os.path.join("/usr/mpi/gcc/openmpi-1.6.5/bin/mpiexec")
